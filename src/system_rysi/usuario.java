...
public class usuario extends javax.swing.JPanel {
    ...
    cUsuario clase_usuario = new cUsuario();
    DefaultTableModel m;
    private Component usuario;
    String id_usuario_global = "";
    String band_alias = "";
    String perfil_usuario_index = "";

    public usuario(String controlador, String DSN, String user, String password, int id_empresa, String perfil_usuario) {
    ...
    private void Mostrar_tabla_usuario() {
        try {
            ResultSet r = sentencia.executeQuery("select id_usuario, alias from tusuario where id_empresa = '" + id_empresa_index + "'order by alias asc");
            ...
            while (r.next()) {
                fila[0] = r.getString("id_usuario").trim();
                fila[1] = r.getString("alias").trim();
            ...
            TableColumn columna2 = tabla_general.getColumn("Alias");
            columna2.setPreferredWidth(300);

        } catch (SQLException ex) {
            JOptionPane.showMessageDialog(null, "Ocurrio al cargar la tabla Usuario - " + ex, "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void activar_caja_texto(String funcion) {
        if (funcion.equals("editable")) {
            ...
            txt_alias.setEditable(true);
            txt_contrasenia.setEditable(true);
            //cbo_perfil.setEditable(true);
            cbo_perfil.setEnabled(true);
        }

        if (funcion.equals("no_editable")) {
            txt_nombre.setEditable(false);
            txt_alias.setEditable(false);
            txt_contrasenia.setEditable(false);
            cbo_perfil.setEditable(false);
    ...
    private void crear_usuario(String nombre, String alias, String contrasenia, String perfil, int id_empresa) {
        System.out.println("\nejecutandose la función: crear_usuario");

        if (clase_usuario.Usuario_crear(nombre, alias, contrasenia, perfil, id_empresa)) {
            System.out.println("\nEl usuario se logró registrar exitosamente!");
            ...
            System.out.println("actualizamos tabla usuario");
            Mostrar_tabla_usuario();

            JOptionPane.showMessageDialog(null, "El usuario se registro exitosamente!");

        } else {
            JOptionPane.showMessageDialog(null, "Ocurrio al crear el Usuario. \nPor favor inténtelo nuevamente.", "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }

    private void modificar_usuario(int id_usuario, String nombre, String alias, String contrasenia, String perfil, int id_empresa) {
        System.out.println("ejecutandose la función: modificar_usuario");

        if (clase_usuario.Usuario_modificar(id_usuario, nombre, alias, contrasenia, perfil, id_empresa)) {
            System.out.println("El usuario se logró modificar exitosamente!");
            ...
            JOptionPane.showMessageDialog(null, "Ocurrio al modificar el Usuario. \nPor favor inténtelo nuevamente.", "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void initComponents() {
        ...
        jLabel1.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/usuario_32_32.png"))); // NOI18N
        ...
        norteLayout.setHorizontalGroup(
            ...
            .addComponent(lbl_titulo, javax.swing.GroupLayout.DEFAULT_SIZE, 636, Short.MAX_VALUE)
        ...
        txt_alias.setEditable(false);
        txt_alias.setBackground(new java.awt.Color(255, 255, 255));
        txt_alias.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        txt_alias.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                txt_aliasKeyTyped(evt);
            }
        });

        txt_contrasenia.setEditable(false);
        txt_contrasenia.setBackground(new java.awt.Color(255, 255, 255));
        txt_contrasenia.setFont(new java.awt.Font("Tahoma", 0, 12)); // NOI18N
        ...
        cbo_perfil.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "Administrador", "Facturacion", "Cotizacion", "Almacen", "Almacen y Facturacion" }));
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel11Layout.createSequentialGroup()
                ...
                .addGroup(jPanel11Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    .addGroup(jPanel11Layout.createSequentialGroup()
                        .addGroup(jPanel11Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING, false)
                            .addComponent(cbo_perfil, javax.swing.GroupLayout.Alignment.LEADING, 0, 173, Short.MAX_VALUE)
    ...
    private void btn_creacionActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_creacionActionPerformed
        ...
        dialog_fecha_creacion.setLocationRelativeTo(usuario);
    ...
    private void txt_buscarKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_buscarKeyReleased
        ...
        try {
            System.out.println("select id_usuario, alias from tusuario where (nombre like '%" + bus + "%'  or alias like '%" + bus + "%' or perfil like '%" + bus + "%')  and id_empresa = '" + id_empresa_index + "' order by alias asc");            
            r = sentencia.executeQuery("select id_usuario, alias from tusuario where (nombre like '%" + bus + "%'  or alias like '%" + bus + "%' or perfil like '%" + bus + "%')  and id_empresa = '" + id_empresa_index + "' order by alias asc");
            DefaultTableModel modelo = new DefaultTableModel();
            modelo.addColumn("");
            modelo.addColumn("Alias");
    ...
    private void btn_modificarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_modificarActionPerformed
        ...
        if (fil == -1) {
            ...
            System.out.println("cambiando el titulo");
            lbl_titulo.setText("Modificar Usuario");
            ...
            band_alias = txt_alias.getText().trim();
        }
        ...
        try {
            ...
            if (fila == -1) {
                ...
                if (respuesta == JOptionPane.YES_OPTION) {
                    ...
                    id_usuario = Integer.parseInt((String) m.getValueAt(fila, 0));
                    System.out.println("el id_usuario que se eliminara es: " + id_usuario);
                    clase_usuario.Usuario_eliminar(id_usuario);

                    System.out.println("El usuario se logró eliminar exitosamente!");
                    System.out.println("ocultar el panel de detalle de datos");
                    Panel_detalle.setVisible(false);
            ...
            JOptionPane.showMessageDialog(null, "Ocurrio al eliminar al Usuario.\n Por favor intentelo nuevamente", "ERROR", JOptionPane.ERROR_MESSAGE);
        }
    }//GEN-LAST:event_btn_eliminarActionPerformed

    private void btn_cancelarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_cancelarActionPerformed
        int respuesta;
        respuesta = JOptionPane.showConfirmDialog(null, "¿Está seguro que desea Cancelar el registro del Usuario?", "Cancelar", JOptionPane.YES_NO_OPTION);
        if (respuesta == JOptionPane.YES_OPTION) {

            System.out.println("\nCancelar creacion del Usuario");
            System.out.println("==============================");
    ...
    private void btn_guardarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_guardarActionPerformed
        ...
        String perfil = cbo_perfil.getSelectedItem().toString();
        int id_empresa = id_empresa_index;

        if (txt_nombre.getText().length() == 0 || txt_alias.getText().length() == 0 || txt_contrasenia.getText().length() == 0) {
            System.out.println("\nFalta ingresar los campos Nombre, Alias y Contraseña");
            JOptionPane.showMessageDialog(null, "Los Campos Nombre, Alias y Contraseña son necesarios \n Por favor ingrese estos campos.", "ERROR", JOptionPane.ERROR_MESSAGE);
        } else {
            if (txt_contrasenia.getText().length() < 8) {
                System.out.println("\nLa contraseña tiene menos de 8 digitos");
                JOptionPane.showMessageDialog(null, "La contraseña tiene que ser de 8 digitos a mas.\n Por favor ingrese nuevamente la Contraseña", "ERROR", JOptionPane.ERROR_MESSAGE);
            } else {
                ...
                if (crear0_modificar1 == 0) {
                    ...
                    if (clase_usuario.alias_existente(alias, id_empresa) > 0) {
                        System.out.println("El Alias ya se encuentra registrado.");
                        JOptionPane.showMessageDialog(null, "El Alias ingresado ya se encuentra Registra. \n Por favor ingrese otro.", "ERROR", JOptionPane.ERROR_MESSAGE);
                    } else {
                        System.out.println("llamamos a la funcion crear_usuario");
                        crear_usuario(nombre, alias, contrasenia, perfil, id_empresa);
                    }
                } else {
                    System.out.println("crear0_modificar1= " + crear0_modificar1);

                    if (band_alias.equals(txt_alias.getText().trim())) {

                        System.out.println("El Alias es igual, se procede con la modificacion");
                        int id_usuario = Integer.parseInt(id_usuario_global);
                        System.out.println("llamamos a la funcion modificar_usuario");
                        modificar_usuario(id_usuario, nombre, alias, contrasenia, perfil, id_empresa);

                    } else {
                        System.out.println("El Alias no es igual, se procede a aplicar restricciones");
    ...
    private void tabla_generalMouseReleased(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_tabla_generalMouseReleased
        ...
        fila = tabla_general.getSelectedRow();
        m = (DefaultTableModel) tabla_general.getModel();
        id_usuario = (String) m.getValueAt(fila, 0);
        System.out.println("el id seleccionado es: " + id_usuario);
        id_usuario_global = id_usuario;

        String perfil = "";

        try {
            ResultSet r = sentencia.executeQuery("select nombre, alias, contrasenia, perfil, id_empresa, f_creacion, f_modificacion from tusuario where id_usuario=" + id_usuario + "");
            while (r.next()) {
                ...
                txt_alias.setText(r.getString("alias").trim());
                System.out.println(txt_alias.getText().trim());

                txt_contrasenia.setText(r.getString("contrasenia").trim());
                System.out.println(txt_contrasenia.getText().trim());

                perfil = r.getString("perfil").trim();
            ...
            JOptionPane.showMessageDialog(null, "Ocurrio al cargar los datos del usuario", "ERROR", JOptionPane.ERROR_MESSAGE);
        }

        DefaultComboBoxModel cbo_per = new DefaultComboBoxModel();
        cbo_per.addElement(perfil);

        String ADM = "Administrador";
        String FAC = "Facturas";
        String GUI = "Guias";
        String FACGUI = "Facturas y Guias";
        String COM = "Compras";
        String SM = "Salidad de Materiales";
        String COMSM = "Compras y Salidas de Materiales";
        String RE = "Reportes";
        String SL = "Solo Lectura";


        if (perfil.equals(ADM)) {
            cbo_per.addElement(FAC);
            cbo_per.addElement(GUI);
            cbo_per.addElement(FACGUI);
            cbo_per.addElement(COM);
            cbo_per.addElement(SM);
            cbo_per.addElement(COMSM);
            cbo_per.addElement(RE);
            cbo_per.addElement(SL);
        } else {
            if (perfil.equals(FAC)) {
                cbo_per.addElement(ADM);
                cbo_per.addElement(GUI);
                cbo_per.addElement(FACGUI);
                cbo_per.addElement(COM);
                cbo_per.addElement(SM);
        ...
        cbo_perfil.setModel(cbo_per);
        cbo_perfil.setEnabled(false);
    ...
    private void txt_aliasKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_aliasKeyTyped
        JTextField caja = txt_alias;
        int limite = 15;
        tamaño_de_caja(caja, evt, limite);
    }//GEN-LAST:event_txt_aliasKeyTyped

    private void txt_contraseniaKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txt_contraseniaKeyTyped
        JTextField caja = txt_contrasenia;
        int limite = 500;
        tamaño_de_caja(caja, evt, limite);
    ...
    private javax.swing.JComboBox cbo_perfil;
